...
(define mt-env empty)
(define (extend-env [env : Env] [x : Symbol] [v : Promise]) : Env
  (cons (bind x v) env))
(define (lookup-env [n : Symbol] [env : Env]) : Promise
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) e2 env)]))

(define (apply [v1 : Value] [e2 : Exp] [e : Env]) : Value
  (type-case Value v1
    [(funV x b env)
     (eval b (extend-env env x (delay e2 e)))]
