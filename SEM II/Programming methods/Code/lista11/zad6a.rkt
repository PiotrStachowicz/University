...
(define (parse-appE f xs)
  (cond [(empty? xs) (error 'parse "invalid input")]
        [(empty? (rest xs)) (appE (parse f) (parse (first xs)))]
        [else (appE (parse-appE f (rest xs)) (parse (first xs)))]))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda {SYMBOL ...} ANY} s)
     (foldr (lambda (x y) (lamE (s-exp->symbol x) y))
    ...
    [(s-exp-match? `{ANY ANY ...} s)
     (parse-appE (first (s-exp->list s)) (reverse (rest (s-exp->list s))))]
