...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [xs : (Listof (Symbol * Exp))] [body : Exp])
  (let*E [xs : (Listof (Symbol * Exp))] [body : Exp]))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{let [(SYMBOL ANY)...] ANY} s)
     (letE (map (lambda (x) (pair (s-exp->symbol (first (s-exp->list x))) (parse (second (s-exp->list x)))))
                (s-exp->list (second (s-exp->list s))))
           (parse (third (s-exp->list s))))]
    [(s-exp-match? `{let* [(SYMBOL ANY)...] ANY} s)
     (let*E (map (lambda (x) (pair (s-exp->symbol (first (s-exp->list x))) (parse (second (s-exp->list x)))))
                 (s-exp->list (second (s-exp->list s))))
           (parse (third (s-exp->list s))))]
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE xs body)
     (eval body (foldl (lambda (x y) (extend-env y (fst x) (eval (snd x) env))) env xs))]
    [(let*E xs body)
     (eval body (foldl (lambda (x y) (extend-env y (fst x) (eval (snd x) y))) env xs))]))
