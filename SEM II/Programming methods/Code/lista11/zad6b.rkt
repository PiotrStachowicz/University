...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda {SYMBOL ...} ANY} s)
     (lamE (map (lambda (x) (s-exp->symbol x)) (s-exp->list (second (s-exp->list s)))) (parse (third (s-exp->list s))))]
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (eval-lamE x b env)]
    [(appE e1 exs)
     (eval-appE (eval e1 env) exs env)]))

(define (eval-appE e xs env)
  (cond
    [(empty? xs) (error 'eval-appE "syntax error!")]
    [(empty? (rest xs)) (apply e (eval (first xs) env))]
    [else (eval-appE (apply e (eval (first xs) env)) (rest xs) env)]))
...
(define (eval-lamE xs b env)
  (cond
    [(empty? xs) (error 'eval-lamE "syntax error!")]
    [(empty? (rest xs)) (funV (λ (v) (eval b (extend-env env (first xs) v))))]
    [else (funV (λ (v) (eval-lamE (rest xs) b (extend-env env (first xs) v))))]))
