...
(define (remove-duplicates xs)
  (if (empty? xs) empty
      (if (member (first xs) (rest xs))
          (remove-duplicates (rest xs))
          (cons (first xs) (remove-duplicates (rest xs))))))

(define (remove x xs)
  (if (empty? xs)
      empty
      (if (eq? x (first xs)) (remove x (rest xs))
          (cons (first xs) (remove x (rest xs))))))

(define (fv [e : Exp]) : (Listof Symbol)
  (type-case Exp e
    [(numE n) empty]
    [(opE op l r) (remove-duplicates (append (fv l) (fv r)))]
    [(ifE b l r) (remove-duplicates (append (fv b) (append (fv l) (fv r))))] 
    [(varE x) (list x)]
    [(letE x e1 e2) (remove-duplicates (remove x (append (fv e1) (fv e2))))]
    [(lamE x e) (remove x (fv e))]
    [(appE e1 e2) (remove-duplicates (append (fv e1) (fv e2)))]))
