...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(funVA b env) (evalA b (extend-envA env v2))]
...
(define (evalA [a : ExpA] [env : (EnvA Value)]) : Value
  (type-case ExpA a
    ...
    [(lamA b)
       (funVA b env)]
    [(appA b arg)
     (apply (evalA b env) (evalA arg env))]
    [(letA e1 e2)
     (let ([v1 (evalA e1 env)])
...
(define (translate [e : Exp] [env : (EnvA Symbol)]) : ExpA
  (type-case Exp e
    ...
    [(appE b1 arg)
     (appA (translate b1 env) (translate arg env))]
    [(letE x e1 e2)
