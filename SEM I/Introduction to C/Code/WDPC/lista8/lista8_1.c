...
int main(void) {
    ...
    static char word[LENGHT];
    int ch;
    int i = 0;
    while((ch = fgetc(stdin))!=EOF){
        if(ch == '\n'){
            insert(root,word);
            memset(word,0,LENGHT);
            i = 0;
        } else{
            word[i] = ch;
            i++;
...
void insert(Node* root, char* word) {
    ...
    for (int i = 0; i < strlen(word); i++) {
        ...
        int index = word[i] - 'a';
        if (temp->edges[index] == NULL) {
            temp->edges[index] = new_node();
        }
...
void print_trie(Node* root) {
    static unsigned char fix[LENGHT];
    print_trie_rec(root, fix, 0);
}
void free_node(Node* node){
    for(int i=0; i<26; i++) {
        if (node->edges[i] != NULL) {
            free_node(node->edges[i]);
        }
    }
    free(node);
}